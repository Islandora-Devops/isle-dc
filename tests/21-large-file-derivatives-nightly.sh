...
TESTCAFE_IMAGE=testcafe/testcafe:"${TESTCAFE_VERSION}"
...
# docker-compose exec -T drupal bash -lc "set -vex ; drush en jwt_auth_issuer && apk add jq && printf '%s-' $(date '+%s') > /tmp/calc ; curl -s -u admin:password http://islandora-idc.traefik.me/jwt/token  | jq .token |cut -f 2 -d "."|base64 -d 2>/dev/null|awk 'BEGIN {RS=\",\";FS=\":\";} /exp/ { print \$2\"\\n\"}' | sed -e '/^$/d' >> /tmp/calc ; cat /tmp/calc; bc < /tmp/calc |sed -e s/-// > /tmp/res ; [ 14400 -le \`cat /tmp/res\` ]"
echo "Skipping 21-large-file-derivatives-nightly.sh tests"