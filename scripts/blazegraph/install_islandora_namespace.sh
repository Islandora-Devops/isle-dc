...
curl -X POST -H "Content-Type: text/plain" --data-binary @/RWStore.properties http://localhost:8080/bigdata/namespace
...
curl -X POST -H "Content-Type: text/plain" --data-binary @/inference.nt http://localhost:8080/bigdata/namespace/islandora/sparql
